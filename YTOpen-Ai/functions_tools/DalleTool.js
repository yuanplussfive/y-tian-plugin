...
*/
 ...
 async handleOtherModels(prompt, model, e) {
   ...
   try {
     ...
     if (!imageArray) {
       ...
       message = '生成失败了，大概率提示词违规，请修改提示词稍后尝试';
     } else {
       ...
       if (urls && urls.length > 0) {
         ...
         message = '生成失败了，没有提取到图片链接，请稍后再试！';
       }
     }
   } catch (error) {
     console.log('其他模型处理错误:', error);
     message = '生成失败了，可能负载过高，请稍后再试！';
   }
 ...
 async zaiwenDraw(prompt, model, size) {
   ...
   if (!modelName) {
     message = '不支持的模型类型';
     return { success: false, message, imageUrl: null, prompt, model, size };
   }

   try {
     imageUrl = await this.zaiwen_drawing(modelName, prompt, ratio);
     if (imageUrl) {
       success = true;
       message = '图片生成成功！';
 ...
 async func(opts, e) {
   ...
   if (!prompt) {
     ...
     return { success: false, message, imageUrl: null, imageUrls: [], prompt, model, size };
   }

   // Zaiwen平台模型处理
   if (Object.keys(this.zaiwenModels).includes(model)) {
     const zaiwenResult = await this.zaiwenDraw(prompt, model, size);
     return zaiwenResult; // 直接返回 zaiwenDraw 的结果
   }

   // 处理其他模型
   if (['jimeng-2.1'].includes(model)) {
     const otherModelResult = await this.handleOtherModels(prompt, model, e);
     return otherModelResult; // 直接返回 handleOtherModels 的结果
   }
   ...
   try {
     ...
     if (response) {
       ...
       dalleImageUrl = links.length > 0 ? links[0] : null;
     }

     if (!dalleImageUrl) {
       dalleImageUrl = await this.dalle(prompt, model);
     }
     ...
     if (dalleImageUrl) {
       ...
       await e.reply(segment.image(dalleImageUrl));
     } else {
       message = '绘图失败，请检查提示词或稍后再试。';
     }
   } catch (error) {
     console.log('DALL-E 处理错误:', error);
     message = '绘图失败，请检查提示词或稍后再试。';
   }
