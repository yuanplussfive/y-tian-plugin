...
import { YTOtherModels, getBase64Image } from '../../utils/fileUtils.js';
...
export class GoogleImageAnalysisTool extends AbstractTool {
    constructor() {
        ...
        this.description = '使用Google的Gemini进行图像分析, 当用户需要分析、处理、识别图片内容，或者询问图片相关信息时使用此工具。支持多图片分析，可提取图片中的文字信息并进行理解分析。注意：所有图片URL必须保持完整原始形式，不得修改或简化URL参数。';
        this.parameters = {
            type: "object",
            properties: {
                prompt: {
    ...
    async func(opts, e) {
        try {
            ...
            const analysis = await YTOtherModels(history, 'gemini-2.0-flash-exp');
