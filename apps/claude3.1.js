...
c = await c.json()
console.log(c)
if(c.error){e.reply("当前预设过长"),true}
let ts = await c.message.ts
let answer;
do{
answer = await fetch(`https://slack.com/api/conversations.replies?channel=${channel}&ts=${ts}&pretty=1`, {
  "headers": {
    "content-type": "multipart/form-data; boundary=----WebKitFormBoundaryTotJQ9kaNkT7dchz",
    "cookie": `d=${d}`
  },
...
//console.log(history[e.user_id])
do{
answer = await fetch(`https://slack.com/api/conversations.replies?channel=${channel}&ts=${history[e.user_id]}&pretty=1&oldest=${ts}`, {
  "headers": {
    "content-type": "multipart/form-data; boundary=----WebKitFormBoundaryTotJQ9kaNkT7dchz",
    "cookie": `d=${d}`
  },
