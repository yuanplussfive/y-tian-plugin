...
let dirpath2 = dirpath + '/drawing.yaml'
if(!fs.existsSync(dirpath2)){
fs.writeFileSync(dirpath2,'cookie: "satoken=xxxxxx; token=xxxxx"',"utf-8")
}
...
weight = msg/100
console.log(weight)
e.reply(`更改成功,现在lora权重为${msg}%`,true)
return true
}else{e.reply("错误的取值范围",true);return false}
}
async drawinghelp(e){
let html = 
`<!DOCTYPE html>
...
<div class="branch">
      <h2>/lora权重+数字(1到100)</h2>
      <p>调整根据搜索的lora画图相似度</p>
    </div>
<div class="branch">
      <h2>/清空lora</h2>
      <p>清空当前所有的lora</p>
