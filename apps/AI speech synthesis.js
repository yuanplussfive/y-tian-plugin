...
import sis from "@huaweicloud/huaweicloud-sdk-sis"
let kl = ""
import fs from 'fs';
let msg = ""
let ying = "zhilingfa"
...
if(p==36){
musical = 1
e.reply(`已切换为华为云提供的chinese_huaxiaoru_common`)
return
}
}
async list(e){
...
"35,标准女童佚佚	anonyg	童声，传统","\n","免费18w字节,用完即停","\n","36,华为云提供chinese_huaxiaoru_common"]
e.reply(msg)
}
async m(e) {
...
if(musical==0){
console.log(ying)
let url = `https://dds.dui.ai/runtime/v1/synthesize?voiceId=${ying}&text=${Msg}&speed=0.8&volume=150&audioType=wav`
e.reply([segment.record(url)])
return
}
if(musical==1){
console.log('华为云')
const ak = "RFWY17JW3AFRPEXUSC0R";
const sk = "SwNcfB2W3HJM0U67HOzEti4VxMunWy5FYoioriSu";
const endpoint = "https://sis-ext.cn-north-4.myhuaweicloud.com";
const project_id = "c9f6270ceab84d58b4456d2cea7dfe10";
...
const client = sis.SisClient.newBuilder()
.withCredential(credentials)
.withEndpoint(endpoint)
                            
.build();
const request = new sis.RunTtsRequest();
const body = new sis.PostCustomTTSReq();
const configbody = new sis.TtsConfig();
configbody.withAudioFormat("mp3")
 .withProperty("chinese_huaxiaoru_common");
body.withConfig(configbody);
body.withText(`${Msg}`);
request.withBody(body);
const result = client.runTts(request);
result.then(result => {
let data = JSON.parse(JSON.stringify(result))
console.log('请求成功')
//console.log(data.result.data)
let y = `${data.result.data}`
var filename = `${_path}/resources/1.mp3`;
var Data = atob(y);
