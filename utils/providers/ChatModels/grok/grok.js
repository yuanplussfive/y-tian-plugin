...
/**
* 调用 Grok API，支持轮询密钥，并在请求失败时尝试使用其他密钥。
* @param {Array} messages - 消息数组，传递给 Grok API。
* @param {string} model - 模型名称，传递给 Grok API。
* @returns {Promise<string|null>} - 如果成功，返回 API 的输出字符串；否则，返回 null。
*/
export const grok = async (messages, model) => {
   const configPath = path.join(process.cwd(), 'plugins/y-tian-plugin/config/message.yaml'); // 构建配置文件路径
   let config = {}; // 初始化配置对象
       ...
       try {
           ...
           const response = await fetch(url, {
               ...
               headers: {
                   ...
                   'Authorization': key // 设置 Authorization 头部，使用当前密钥
               },
